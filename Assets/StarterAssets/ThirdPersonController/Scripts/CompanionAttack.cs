...
public class CompanionAttack : MonoBehaviour
{
    ...
    private void OnTriggerEnter(Collider myspace)
    {
        if (myspace.TryGetComponent<Attackable>(out Attackable attackable))
        {
    ...
    private void OnTriggerExit(Collider myspace)
    {
        if (myspace.TryGetComponent<Attackable>(out Attackable attackable))
        {
